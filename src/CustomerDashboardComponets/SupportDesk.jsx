...
const FRANCHISE_CONTACT = {
  ...
  supportEmail: "torontowest@waleshealthcare.com",
};

// value = what the API stores, label = what the customer sees
const CATEGORY_OPTIONS = [
  { value: "Care Coordination", label: "Care & scheduling" },
  { value: "Financial", label: "Billing & invoices" },
  { value: "Technical", label: "Portal or technical issue" },
  { value: "Administrative", label: "Account & paperwork" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/60 p-10 text-center">
        ...
        <h3 className="text-lg font-black text-slate-900">How can we help?</h3>
        <p className="max-w-md text-xs text-slate-500">
          Reach your franchise office directly, or send us a message and we'll get back to you shortly.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs">
          ...
          <p className="text-sm font-bold text-slate-800">Call Your Franchise</p>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs">
          ...
          <p className="text-xs text-slate-400">{FRANCHISE_CONTACT.supportEmail}</p>
        </div>
      </div>
      ...
      >
        ...
        <div className="space-y-3">
          <input
            ...
            required
            maxLength={255}
            placeholder="Subject (e.g. Change my Thursday visit)"
            value={form.subject}
              ...
              <li key={ticket.id} className="flex items-center justify-between gap-3 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] font-semibold text-slate-400">
                    {ticket.reference} • {categoryLabel(ticket.category)} • {formatDateTime(ticket.created_at)}
                  </p>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-900 px-5 py-4 text-white sm:px-6">
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-widest text-teal-400">
                  {activeTicket.reference} • {categoryLabel(activeTicket.category)}
                </p>
            ...
            <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs">
              <div className="min-w-0 truncate">
                ...
                <span className="font-semibold text-slate-800">
                  {activeTicket.assigned_to_name || "Your franchise office"}
                </span>
              </div>
              <span
